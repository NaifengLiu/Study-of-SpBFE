...
if __name__ == "__main__":
	...
	for treenum in range(len(alltreesn[n])):
		...
		if True:
			...
			# print(optcost)
			BooleanFunctions.index_used = -1
			f = BooleanFunctions.Formula(BooleanFunctions.class_type(tree))
			# print(treenum)
			# print(f.f.to_string())
			# if f.f.to_string()[:6] == 'AND(OR':
			if f.f.to_string() == 'AND(OR(x[0],x[1]),OR(x[2],AND(x[3],x[4]),AND(x[5],x[6])))':
				f.show()
				strategy.pprint()
				classes, paths = getsiblings(tree, [])
				print(classes)
	...
	# all_trees = alltreesn[8][500:]
	# for i in tqdm(range(len(all_trees))):
	# 	tree = all_trees[i]
	# 	numvar = 8
	# 	# print(tree)
	# 	costs, probs = getunit(tree, p)
