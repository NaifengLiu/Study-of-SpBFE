...
# you can either
f = Formula(AND([OR(['x0', 'x1']), OR([AND(['x2', 'x3']), 'x4', AND(['x5', 'x6'])])]))
example = Tree(7, f, greedy_influence, [.5] * 7)
print(example.calculate_expected_cost())
example.print()
#
# or
# f = Formula(OR(['x0', AND(['x1', 'x2']), AND([OR(['x3', 'x4']), OR(['x5', AND(['x6', 'x7'])])])]))

n = 8
p = .5
read_once_fs = generate_all_read_once_functions(n)
for index in tqdm(range(len(read_once_fs[n]))):
	if index == 453:
		each = read_once_fs[n][index]
		string = each[0].f.to_string()
		for i in range(n):
			string = string.replace('x[' + str(i) + ']', "'x" + str(i) + "'")
		string = string.replace('(', '([')
		string = string.replace(')', '])')
		f = Formula(eval(string))
		greedy_inf = Tree(n, f, greedy_influence, [.5] * n)
		greedy_inf_cost = greedy_inf.calculate_expected_cost()
		# print(greedy_inf_cost)
		tree = each[1]
		costs, probs = DynProg.getunit(tree, p)
		expectedcost, tests = DynProg.getoptimalcost(tree, costs, probs)
		optcost = expectedcost[list(tests.keys())[-1]]
		# print(optcost)
		# print()
		if greedy_inf_cost != optcost:
			print(string)
			print(greedy_inf_cost, optcost)
			DynProg.currentnum = 0
			tree = DynProg.fixtree(tree)
			strategy = DynProg.buildstrategy(tree, expectedcost, tests)
			strategy.pprint()
